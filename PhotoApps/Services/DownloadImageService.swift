...
protocol DownloadImageServiceProtocol {
    func handlerResponse (
        data: Data?,
        response: URLResponse?) -> UIImage?

    func downloadimage (
        serverId: String,
        photoId: String,
        secret: String,
        resolution: String) -> AnyPublisher<UIImage?, Error>
}
   ...
   let url = URL(string: "https://live.staticflickr.com/.jpg")!
    ...
    func downloadimage(
        ...
        URLSession.shared.dataTaskPublisher(
            for: URL(string: "https://live.staticflickr.com/\(serverId)/\(photoId)_\(secret)_\(resolution).jpg")!)
            .map(handlerResponse)
            .mapError({$0})
            .eraseToAnyPublisher()
